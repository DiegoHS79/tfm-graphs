...
def normalize_data(sf: Series) -> dict:
    ...
    print("-" * 100)
    direction = f"{direction}, {sf.loc['city']}, ({prov})"
    if (
...
COMISARIAS = Path("../scrapy_data/spiders/data/comisarias.csv")
COMISARIAS_FINAL = Path("data/comisarias.csv")
if not COMISARIAS_FINAL.exists():
    df = pd.read_csv(COMISARIAS, sep="\t")
    ...
    df_final.to_csv(COMISARIAS_FINAL, index=False, sep="\t")


if COMISARIAS_FINAL.exists:
    ...
    df_tmp = pd.read_csv(COMISARIAS_FINAL, sep="\t")
